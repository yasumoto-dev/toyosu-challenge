...
| 2 | 設定パラメータは config の property に集約する | 各ストリームの property に設定値を散らさない。ただしセンサ固有情報は例外（下記） |
...
#### 決定2の補足: モジュール固有の property は各ストリームに載せる（2026-09-21 決定）

**YAML に書ける「設定値」は config の property に集約する。一方、そのモジュールが
実行時に初めて知る「固有情報」は、モジュール自身のストリームの property に載せてよい。**

例: `urg_fs.property`（`urg_property`）に載せるセンサ情報。型番・シリアル番号・
`step_front` / `step_resolution` / `revolution` は、起動時に PP / VV コマンドで
センサから取得して初めて分かる値であり、YAML に人が書く値ではない。config に
置くと「人が書いた値」と「センサが名乗った値」が混ざり、食い違ったときに
どちらが真か判断できなくなる。

判断基準: **その値を書くのは人か、機械か。** 人が書くなら config、機械が名乗るなら
そのストリームの property。
...
| 対象 | 書き方 | 例 |
|---|---|---|
| SSM に流す構造体・パラメータ構造体 | スネークケース | `urg_fs` / `urg_property` / `robot_param` |
| クラス | キャメルケース | `lidarHokuyo` |
| 関数・メソッド | キャメルケース | `getPointCloudData()` / `setupSSM()` |
| ファイルスコープ変数 | `g` + キャメルケース | `gShutOff` / `gStart` |
| マクロ・定数 | 大文字スネークケース | `SNAME_URG` / `URG_DATA_MAX` |

構造体とクラスで書き分ける理由: **構造体は「SSM 越しに他プロセスへ渡るデータ」、
クラスは「そのプロセス内だけで動く道具」**であり、役割が違う。見た目で区別できると、
その型が共有メモリに載るかどうかが読んだ瞬間に分かる。

TC2025 はクラスがスネークケース（`lidar_Hokuyo`）で構造体と区別が付かなかった。
...
- **ビューアを作ってドリフト量を測る実験をする**（2026-09-18 に方針として合意）。
  ...
  可視化の手段は §12 で決定済み。
  実験内容（周回して始点に戻る、直進 10m、その場回転 10 周など）は未決。
- `course.start_pose` の実測値（§8 の実測値表を参照。**未確定**）

---

## 12. 可視化の設計（2026-09-20）

### 可視化は Python (matplotlib) を使う

- `odom_logger` が CSV を標準出力に流し、`scripts/plot_odom.py` が描画・解析する。
- TC2025 の `odm-viewer` は gnuplot 依存だが、ここは従わない。
  理由: ビューアの目的はドリフト量の数値計測であり、描画だけでなく配列計算
  （始点残差、横ずれ、`yaw` のアンラップ）と地図 PCD との重ね描きが必要になる。
  gnuplot でこれをやると結局 Python の前処理が要り、二重管理になる。
- 可視化はオフライン解析であり、走行に影響しないため §2 の TC2025 準拠から外す。
- 依存は apt の `python3-matplotlib` / `python3-numpy`。venv は不要
  （必要になったら `python3 -m venv --system-site-packages .venv`）。
- 走行中のリアルタイム監視は別件。必要になった時点で改めて決める。
  この用途では gnuplot が有利（実機に Python 環境が不要、起動が速い）。

### 検討したが採用しなかった案

| 案 | 却下理由 |
|---|---|
| gnuplot 版 (`scripts/plot_odom.gp`) を残す | 2026-09-20 に作成したが削除した。Python 版と併存させると `odom_gl` の構造体を変えるたびに2箇所直す必要があり、列番号が gnuplot は1始まり・Python は0始まりで食い違うため、片方の直し忘れがエラーにならず「静かに間違ったグラフが出る」。§2 変更点 #5 で CMake の二重管理を避けたのと同じ理由 |

### ログから CSV への変換手順

`logs/<日時>/*.log` は `ssm-logger` が書いたバイナリで、そのままでは読めない。
SSM に戻して `odom_logger` で読み直す。

```
odom_gl.log ──ssm-player──> SSM共有メモリ ──odom_logger──> odom.csv ──plot_odom.py──> グラフ
```

1. 端末A: `ssm-coordinator`
2. 端末B: `./bin/odom_logger > logs/odometry.csv`
3. 端末C: `ssm-player logs/<日時>/odom_gl.log`
4. 再生終了後、端末B を Ctrl-C
5. `./scripts/plot_odom.py logs/odometry.csv`

`ssm-player -x` で早送りできるが、`odom_logger` の `readNew()` が取りこぼす可能性が
あり「データが減った」のか「再生が速すぎた」のか切り分けられなくなるため、等倍で通す。

### 注意点

- **列番号は `odom_logger.cpp` の `printf` と `plot_odom.py` の `COL_X` / `COL_Y` が
  一致している前提。** `odom_gl` の構造体を変えたら両方直す。
- `yaw` が ±π で垂直に飛ぶのは `atan2` の値域であってバグではない。
  ドリフト量の計測で角度誤差を積算する際はアンラップが要る。
